...
const SeedStagesData = require("../../../../../src/seeder/stageSeeder.v2")
    .seedStageData;
...
const LevelStage = require("./transforming-aprroach.model");

const auth = require("../../../../../middleware/auth");
const admin = require("../../../../../middleware/checkAdmin");
const { func } = require("joi");
const { seedStageData } = require("../../../../../src/seeder/stageSeeder.v2");
...
function addSectionToStage(req, res) {
    let lang = req.query.language || "eng";
    let section = {
    ...
    LevelStage.findOneAndUpdate(
        { _id: req.body.stageId, "stages.language": lang },
        { $push: { "stages.$.sections": section } },
        { upsert: false }
    )
        .then((product) => {
            res.status(201).json({
...
function addToolToLevelStage(req, res) {
    ...
    LevelStage.findOneAndUpdate(
        { _id: req.body.stageId, "stages.language": language },
        { $push: { "stages.$.recommendedTools": tool } },
...
function updatSectionInStage(req, res) {
    LevelStage.update(
        ...
        {
            $set: {
                "stages.$[t].sections.$[sec].title": req.body.title,
                "stages.$[t].sections.$[sec].body": req.body.body,
            },
        },
        {
            arrayFilters: [
                { "t.language": req.query.language },
                { "sec._id": req.query.sid },
            ],
            upsert: false,
            new: true,
        }
...
function updatToolInStage(req, res) {
    ...
    LevelStage.update({ _id: req.query.stgId },
        {
            $set: {
                "stages.$[t].recommendedTools.$[tool].title": req.body.title,
                "stages.$[t].recommendedTools.$[tool].redirectUrl": req.body.body,
                "stages.$[t].recommendedTools.$[tool].file": icon,
                "stages.$[t].recommendedTools.$[tool].fileType": req.body.fileType || 'video',
                "stages.$[t].recommendedTools.$[tool].redirectUrl": req.body.redirectUrl
            }
        },
...
function updateStageInfo(req, res) {
    ...
    LevelStage.findOneAndUpdate({ _id: req.params.id, "stages.language": req.query.language }, {
        $set: {
